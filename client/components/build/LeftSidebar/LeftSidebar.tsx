...
import Menu from '@/components/custom/side-menu/menu';
...
const LeftSidebar = () => {
  ...
  const handleTriggerEdit = (e: string) => {
    if (e == 'Edit') {
      setIsEdit(true);
    } else if (e == 'Styles') {
      setIsEdit(false);
  ...
  return (
    ...
    >
      <div className={styles.container}>
        <Menu handleTriggerEdit={handleTriggerEdit} />
        <nav className="overflow-y-scroll">
          ...
          <div className={isEdit ? styles.sections : styles.hidden}>
            {left.map(({ id, icon }) => (
              <Tooltip
                arrow
                key={id}
        ...
        <main>
          <div className={isEdit ? '' : styles.hidden}>
            {sectionsList()}
