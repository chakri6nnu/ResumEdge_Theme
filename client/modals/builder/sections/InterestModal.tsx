...
// import { InterestIcons, InterestIconsFormat } from '@/constants/Interests';
...
const InterestModal: React.FC = () => {
  ...
  // const InterestIconsList: InterestIconsFormat[] = InterestIcons;
  ...
  return (
    ...
    >
      <form className="my-2 grid grid-cols-2 gap-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        />
        {/* <ul className="list-inside list-none leading-loose">
          {InterestIcons.map((intrestItem: InterestIconsFormat, index) => {
            return (
              <li key={index}>
                <img
                  style={{ width: '50px' }}
                  src={`http://localhost/interest-icons/${intrestItem.name}.svg`}
                  alt={intrestItem.name}
                />
              </li>
            );
          })}
