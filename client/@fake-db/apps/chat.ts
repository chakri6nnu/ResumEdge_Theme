...
// ** Types
import { ChatsObj, ContactType, ProfileUserType } from './../../types/apps/chatTypes';
...
mock.onGet('/apps/chat/chats-and-contacts').reply(() => {
  const chatsContacts = data.chats.map((chat: ChatsObj) => {
    const contact: any = data.contacts.find((c: ContactType) => c.id === chat.userId);
...
mock.onGet('/apps/chat/get-chat').reply((config) => {
  ...
  const contact: any = data.contacts.find((c: ContactType) => c.id === userId);
...
mock.onPost('/apps/chat/send-msg').reply((config) => {
  ...
  const response: any = { newMessageData, id: obj.contact.id };
