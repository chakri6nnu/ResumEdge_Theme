...
const Section: React.FC<SectionProps> = ({
  ...
  return (
    <section id={`Resume1_${sectionId}`}>
      <h3 className={clsx(stylesLoc.mainTitle)}>
        <span
          style={{ backgroundColor: isSidebarSection ? 'transparent' : theme.background }}
          className={stylesLoc.mainTitleInner}
        >
          {section.name}
        </span>
        <span
          className={clsx(stylesLoc.mainTitleInnerBar)}
          style={{ borderColor: isSidebarSection ? '#000000' : theme.text }}
        >
          &nbsp;
        </span>
      </h3>
