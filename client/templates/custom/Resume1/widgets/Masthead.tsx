...
export const MastheadSidebar: React.FC = () => {
  ...
  return (
    <div className="col-span-2 grid justify-items-center gap-3">
      ...
      <div className={clsx('flex flex-col gap-2.5  px-4 pt-4', css(`svg { color: ${color} }`))}>
        {email && (
          <h3 className={cx(stylesLoc.mainTitle, 'mt-2', contrast === 'light' ? stylesLoc.textInvert : '')}>
            <span className={clsx(stylesLoc.mainTitleInner)}>Personal</span>
            <span className={clsx(stylesLoc.mainTitleInnerBar)}>&nbsp;</span>
          </h3>
        )}
...
export const MastheadMain: React.FC = () => {
  ...
  const theme: ThemeConfig = useAppSelector((state) => get(state.resume.present, 'metadata.theme', {} as ThemeConfig));
  return (
    <div className="px-4 pt-4" style={{ position: 'relative' }}>
      {summary && (
        <div className={stylesLoc.summary}>
          <h3 className={stylesLoc.mainTitle}>
            <span className={stylesLoc.mainTitleInner}>PROFILE </span>
            <span className={clsx(stylesLoc.mainTitleInnerBar)} style={{ borderColor: theme.text }}>
              &nbsp;
            </span>
          </h3>
      ...
      <div className={stylesLoc.header} style={{ backgroundColor: theme.primary }}>
        <h1 className={stylesLoc.whiteText} style={{ color: theme.text }}>
          {name}
        </h1>
        <p className={stylesLoc.whiteText} style={{ color: theme.text }}>
          {headline}
        </p>
      </div>
    </div>
